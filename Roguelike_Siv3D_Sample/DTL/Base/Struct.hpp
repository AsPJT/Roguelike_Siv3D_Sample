...
namespace dtl {
	inline namespace base { //"dtl::base"名前空間に属する
		...
		struct Coordinate2Dimensional {
			...
			constexpr Coordinate2Dimensional operator+=(const ::dtl::base::Coordinate2Dimensional<Int_>& vec2_) noexcept {
				this->x += vec2_.x;
				this->y += vec2_.y;
				return *this;
			}
			constexpr Coordinate2Dimensional operator+=(const Int_& int_) noexcept {
				this->x += int_;
				this->y += int_;
				return *this;
			}
			...
			constexpr Coordinate2Dimensional operator++() noexcept {
				++this->x;
				++this->y;
				return *this;
			}
