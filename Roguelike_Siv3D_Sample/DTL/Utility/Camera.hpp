...
namespace dtl {
	inline namespace utility { //"dtl::utility"名前空間に属する
		...
		void setCameraLoop(const dtl::MatrixRange& win, const dtl::MatrixVec2& wp, const dtl::Vec2<Float_>& mc, dtl::Vec2 <Int_>& wssp, dtl::Vec2 <Int_>& wsss) noexcept {
			...
			wssp.x = (wsxi == wsx) ? 0 : static_cast<Int_>(-(wsx - wsxi) * (wp.x) - static_cast<Float_>((wsx > 0.0f) ? 0 : static_cast<Int_>(wp.x)));
			wssp.y = (wsyi == wsy) ? 0 : static_cast<Int_>(-(wsy - wsyi) * (wp.y) - static_cast<Float_>((wsy > 0.0f) ? 0 : static_cast<Int_>(wp.y)));
			wsss.x = static_cast<Int_>((wsxi == wsx) ? static_cast<Int_>(wsx) : static_cast<Float_>((wsx > 0.0f) ? static_cast<Int_>(wsx) : static_cast<Int_>(wsx) - 1));
			wsss.y = static_cast<Int_>((wsyi == wsy) ? static_cast<Int_>(wsy) : static_cast<Float_>((wsy > 0.0f) ? static_cast<Int_>(wsy) : static_cast<Int_>(wsy) - 1));
		}
